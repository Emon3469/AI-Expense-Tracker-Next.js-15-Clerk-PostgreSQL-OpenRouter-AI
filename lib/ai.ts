...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    "HTTP-Referer": process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000', // Optional. Site URL for rankings on openrouter.ai.
    "X-Title": "PennyPilot", // Optional. Site title for rankings on openrouter.ai.
  },
});
...
export async function generateExpenseInsights (
    ...
    try{
        ...
        const formattedInsights = insight.map(
...
export async function categorizeExpense(description: string): Promise<string> {
    try{
        ...
        const finalCategory = vaildCategories.includes(category || '') ? category! : 'Other';
