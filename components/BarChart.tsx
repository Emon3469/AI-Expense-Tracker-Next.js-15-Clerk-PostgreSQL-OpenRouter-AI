...
const BarChart = ({ records } : { records: Record[] }) => {
    ...
    const aggregateByDate = (records : Record[]) => {
        ...
        records.forEach((record) => {
            ...
            const dateKey = `${year}-${month}_${day}`;
    ...
    const aggregateData = aggregateByDate(records);
  ...
  const data = {
    ...
    datasets: [
      {
        data: aggregateData.map((item) => item.amount),
        backgroundColor: aggregateData.map((item) => getAmountColor(item.amount).bg),
        borderColor: aggregateData.map((item) => getAmountColor(item.amount).border),
        borderWidth: 1,
        borderRadius: 2,
      },
    ],
    ...
    plugins: {
        ...
        callbacks: {
          label: function (context: { dataIndex: number }) {
            ...
            const item = aggregateData[dataIndex];
