...
import { FileWarning, CloudCheck, SquarePen, Info, TrainFront, Recycle, ArrowBigUpDash, UserStar, BrainCircuit, ArrowBigRightDash} from 'lucide-react'
...
const AIInsights = () => {
    ...
    const handleActionClick = async (insights: InsightData) => {
        if(!insights.action) return;

        const existingAnswer = aiAnswer.find((a) => a.insightId === insights.id)
        if(existingAnswer) {
            setAiAnswer((prev) => prev.filter((a) => a.insightId === insights.id));
            return;
        }
        ...
        try{
            const question = `${insights.title}: ${insights.action}`;

            const answer = await generateInsightAnswer(question);

            setAiAnswer((prev) => prev.map((a) => a.insightId === insights.id ? { ...a, answer, isLoading: false } : a));
        }
        catch(error){
            console.error('❌ Failed to generate AI answer:', error);
            setAiAnswer((prev) =>
                prev.map((a) =>
                    a.insightId === insights.id
                    ? {
                        ...a,
                        answer:
    ...
    const getInsightIcon = (type: string) => {
        switch(type){
            ...
            case 'success': return  <CloudCheck />
            case 'tip': return  <SquarePen />
  ...
  const formatLastUpdated = () => {
    ...
    if(diffMs < 60) return `${diffMins}m ago`;
    ...
    return (
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-xl border border-gray-100/50 dark:border-gray-700/50">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                <div className="w-8 h-8 sm:w-10 bg-gradient-to-br from-emerald-500 via-blue-500 to-teal-500 rounded-xl flex items-center justify-center shadow-lg">
                    <span className="text-white text-sm sm:text-lg"><TrainFront /></span>
                </div>
            ...
            <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700 text-center">
                <div className="flex items-center justify-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                    <div className="w-1.5 h-1.5 bg-emerald-500 dark:bg-emerald-400 rounded-full animate-spin"></div>
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-xl border border-gray-100/50 dark:border-gray-700/50 hover:shadow-2xl">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
                <div className="inline-flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/50 text-emerald-700  dark:text-emerald-300 px-2 py-1 rounded-full text-xs font-medium">
                ...
                <button onClick={loadInsights} className="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-r from-emerald-600 via-blue-500 to-teal-500 hover:from-emerald-700 hover:via-blue-600 hover:to-teal-600 text-white rounded-xl flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
            {
                insights.map((insight) => {
                    const currentAnswer = aiAnswer.find((a) => a.insightId === insight.id);
                           ...
                           <div className="flex items-start justify-between">
                               <div className="flex-1">
                                    <div className="flex items-center gap-2 sm:gap-3 mb-2">
                                      ...
                                      <div className="flex-1">
                                          <h4 className="font-bold text-gray-900 dark:text-gray-900 ">
                                              { insight.title }
                                          </h4>
                                          {
                                  ...
                                  </p>
                                  {
                                    insight.action && (
                                        <div>
                                            <span onClick={() => handleActionClick(insight) }
                                               ...
                                               <span>{insight.action}</span>
                                               {
                                                 currentAnswer?.isLoading ? (
                                                    ...
                                                    <span className="text-xs">
                                                        {
                                                            currentAnswer ? <ArrowBigUpDash /> : <UserStar />
                                                        }
                                                    </span>
                                                 )
                                               }
                                     ...
                                     currentAnswer && (
                                        <div className="mt-3 p-3 bg-white/70 dark:bg-gray-700/70 backdrop-blur-sm rounded-lg border border-gray-200 dark:border-gray-600">
                                            <div className="flex items-start gap-2">
                                                <div className="w-5 h-5 sm:w-6 sm:h-6 bg-gradient-to-br from-emerald-500 via-blue-500 to-teal-500 rounded-lg flex items-center justify-center flex-shrink-0">
                                                    <span className="text-white text-xs"><TrainFront /></span>
                                                </div>
                                                <div className="flex-1">
        ...
        <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <div className="w-5 h-5 sm:w-6 bg-emerald-50 dark:bg-emerald-900/30 rounded-lg flex items-center justify-center">
                        <span className="text-sm"><BrainCircuit /></span>
                    </div>
                    <span className="font-medium text-xs">Powered by AI Analysis</span>
                </div>
                <button onClick={loadInsights} 
                 className="px-3 py-1.5 bg-gradient-to-r from-emerald-600 via-blue-500 to-teal-500 hover:from-emerald-700 hover:via-blue-600 hover:to-teal-600 text-white rounded-lg font-medium text-xs shadow-lg hover:shadow-xl transition-all duration-200"
                >
                    <span className="sm:hidden">Refresh</span>
                    <span className="hidden sm:inline">Refresh Insights <ArrowBigRightDash /></span>
                </button>
            </div>
        </div>
    </div>
